...
describe('FoodList', () => {
  it('renders empty state when no foods', () => {
    render(<FoodList foods={[]} onRemove={vi.fn()} />);
  ...
  it('renders food items with name and category', () => {
    ...
    render(<FoodList foods={foods} onRemove={vi.fn()} />);
    ...
    expect(screen.getAllByText(/AOVE/)).toHaveLength(2);
  });
  ...
  it('calls onRemove with correct index when remove button clicked', async () => {
    ...
    render(<FoodList foods={foods} onRemove={onRemove} />);
  ...
  it('has accessible list label when foods present', () => {
