...
describe('meal distribution', () => {
  describe('4-meal standard (default mealCount=4)', () => {
    function getDayBreakdown(
      plan: import('./planGenerator').WeeklyPlan,
      dayIndex: number,
    ...
    describe('enforceAOVE pure function', () => {
      ...
      it('adds AOVE to LUNCH when missing', () => {
        ...
        const added = result.find(
          (e) => e.mealType === MealType.LUNCH && e.food.category === FoodCategory.OLIVE_OIL,
        );
        expect(added).toBeDefined();
      });

      it('does not crash when no OLIVE_OIL food is available', () => {
        const entries = [{ food: cerealFood, rations: 1, mealType: MealType.BREAKFAST }];
