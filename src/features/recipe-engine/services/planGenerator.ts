...
function buildDailyTemplate(restrictionActive: boolean, mealCount = 4): TemplateSlot[] {
  ...
  const cerealDinner = Math.max(cerealMax - 3, 0) // 2 normally, 1 when restricted

  const baseSlots: TemplateSlot[] = [
    { category: FoodCategory.CEREALS, mealSlots: [
      { meal: MealType.BREAKFAST, rations: 1 },
      { meal: MealType.LUNCH, rations: 2 },
      { meal: MealType.DINNER, rations: cerealDinner },
    ]},
    { category: FoodCategory.VEGETABLES, mealSlots: [
      { meal: MealType.LUNCH, rations: 2 },
      { meal: MealType.DINNER, rations: 1 },
    ]},
    { category: FoodCategory.OLIVE_OIL, mealSlots: [
      { meal: MealType.BREAKFAST, rations: 1 },
  ...
  if (mealCount <= 3) {
    baseSlots.push(
      { category: FoodCategory.FRUITS, mealSlots: [
        { meal: MealType.BREAKFAST, rations: 2 },
      ]},
      { category: FoodCategory.WATER, mealSlots: [
        { meal: MealType.BREAKFAST, rations: 1 },
        { meal: MealType.LUNCH, rations: 2 },
        { meal: MealType.DINNER, rations: 1 },
      ]},
  ...
  const snackCount = mealCount - 3 // B+L+D = 3 base meals
...
export function enforceAOVE(entries: MealEntry[], day: number): MealEntry[] {
  ...
  for (const meal of mainMeals) {
    const hasAOVE = result.some(e => e.mealType === meal && e.food.category === FoodCategory.OLIVE_OIL)
    if (!hasAOVE) {
