...
describe('CulturalMetadataSchema', () => {
  ...
  it('accepts proteinBiologicalValue at boundaries (0 and 100)', () => {
    expect(CulturalMetadataSchema.parse({ proteinBiologicalValue: 0 }).proteinBiologicalValue).toBe(
      0,
    );
    expect(
      CulturalMetadataSchema.parse({ proteinBiologicalValue: 100 }).proteinBiologicalValue,
    ).toBe(100);
  });
});
