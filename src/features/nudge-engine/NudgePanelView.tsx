      ...
      {pending.map((nudge) => (
        ...
        >
          <div className="min-w-0">
            <p className="font-medium text-sm text-stone-800 dark:text-zinc-200">{nudge.title}</p>
            <p className="text-xs text-stone-500 dark:text-zinc-400 mt-0.5">{nudge.body}</p>
          </div>
          <button
            ...
            aria-label={t['nudges.dismissAria'].replace('{title}', nudge.title)}
          >
            ✕
          </button>
        </div>
        ...
        <details className="text-sm">
          ...
          <ul className="mt-2 space-y-1 ml-2">
            {history.map((entry) => (
              ...
              >
                <span className="text-stone-500 dark:text-zinc-400">{entry.title}</span>
                <span className="ml-1">
                  {entry.dismissedAt
                    ? `— ${t['nudges.dismissed']}`
